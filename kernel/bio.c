...
{
  if (!holdingsleep(&b->lock))
    panic("brelse");  // 如果没有持有缓冲区的锁，则触发内核恐慌

  releasesleep(&b->lock);  // 释放缓冲区的锁

  int h = HASH(b->blockno);  // 获取缓冲区哈希桶的索引
  acquire(&bcache.lock[h]);  // 获取对应缓冲区哈希桶的锁
  b->refcnt--;  // 减少缓冲区的引用计数
  release(&bcache.lock[h]);  // 释放对应缓冲区哈希桶的锁
}


void bpin(struct buf *b) {
  acquire(&bcache.lock[HASH(b->blockno)]);  // 获取对应缓冲区哈希桶的锁
  b->refcnt++;  // 增加缓冲区的引用计数
  release(&bcache.lock[HASH(b->blockno)]);  // 释放对应缓冲区哈希桶的锁
}
